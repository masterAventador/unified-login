...
Java 包名 `com.aventador.unifiedlogin` 为待确认项，评审时可改。
...
| 连续登录失败 | 同一邮箱连续失败 5 次锁定 15 分钟；同一 IP 每分钟登录尝试上限 20 次。计数状态保存在应用内存中（一期为单实例部署）；未来多实例部署时需迁移至共享存储，此约束需在部署文档中注明 |
...
| 阶段 | 内容 | 阶段结束时可验证的事实 |
| --- | --- | --- |
| 一 · 地基 | 认证中心骨架、`app_user` 表与迁移、注册、登录页、授权码 + PKCE 流程、demo-web-a | 能在浏览器完成注册并登录 demo-web-a |
