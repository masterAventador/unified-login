...
浏览器里的接入方部署在自己的源上（如 `https://app.<主域名>`），换令牌时要跨源 POST 到
`auth.<主域名>/oauth2/token`。**缺少 CORS 放行时服务端一切正常——令牌照签、状态码 200、
日志无任何异常——但浏览器会把响应整个丢弃，页面只看到 `Failed to fetch`。**
因此它是接入方能否登录成功的必要条件，不是可选的加固项。

这个缺陷是阶段一的真实浏览器验收才暴露出来的：此前所有 MockMvc 层测试全绿，因为
MockMvc 不执行浏览器的同源策略。这也是为什么每个阶段都必须有一道真实链路验收。

实现要求：

- **允许的源从已注册客户端的回调地址推导**，不另立一份名单——回调白名单本就是「哪些源是
  我们承认的接入方」的唯一事实来源，另开一份迟早会与它不同步。
- 只放行 `POST` 与 `Content-Type` 请求头；不开 `allowCredentials`（公有客户端换令牌不需要携带 Cookie）。
- **必须注册为全局 Servlet 过滤器，不能用 `http.cors(...)`**：授权服务器的端点匹配器对令牌端点
  只匹配 POST，浏览器发来的 `OPTIONS /oauth2/token` 预检落不进那条过滤器链，会掉进默认链
  被判为未认证并 302 跳登录页，于是真正的 POST 根本不会发出。
- **⚠ 推导来源时必须只取 http/https 回调**：阶段二接入桌面端与移动端后会出现自定义 scheme
  的回调地址（如 `myapp://callback`），它推导出的「源」没有 host，灌进 CORS 允许列表会导致
  配置非法甚至启动失败。非浏览器客户端本来也不受同源策略约束、不需要 CORS，因此推导时
  应过滤掉非 http/https 的回调。这一点在阶段一尚未触发，但接入原生客户端前必须先处理。
...
| 连续登录失败 | 同一邮箱连续失败 5 次锁定 15 分钟；同一 IP 每分钟登录尝试上限 20 次。**对不存在的邮箱、格式非法的输入、以及已禁用的账号，全部同样计数并锁定**——否则「是否会被锁定」本身就泄漏了账号状态。特别注意：禁用账号抛出的是 `DisabledException`，对应 `AuthenticationFailureDisabledEvent` 而非 `AuthenticationFailureBadCredentialsEvent`，若只订阅后者，禁用账号将永不被锁，攻击者连敲六次即可据「锁没锁」识别出「账号存在但已禁用」。计数须覆盖所有**与账号状态相关**的失败事件；基础设施类失败（`AuthenticationFailureServiceExceptionEvent`、`AuthenticationFailureProviderNotFoundEvent`）不计数，否则数据库故障会把正常用户批量锁死。计数状态保存在应用内存中（一期为单实例部署）；未来多实例部署时需迁移至共享存储，此约束需在部署文档中注明。**另一项已知限制**：计数缓存有条目上限并按大小淘汰，分布式攻击者用大量伪造邮箱撑爆缓存可把目标账号已生效的锁定记录挤出去，从而绕过邮箱锁定。单 IP 下被每分钟 20 次节流，撑爆所需时间远超锁定时长因而不成立；多 IP 场景下成立。迁移到共享存储时需一并处理（按邮箱分片或不设总量上限） |
| 账号已禁用 | 登录直接拒绝；持有的 refresh token 也无法换取新 token。**实现要点**：刷新流程用的是首次登录时序列化进 `oauth2_authorization` 的 principal 快照，框架自带的刷新 provider **从不回查用户表**，因此必须把它**替换**成带账号回查的版本。注意不能只在令牌端点追加一个 provider——`ProviderManager` 对普通 `AuthenticationException` 只记 `lastException` 后继续遍历，框架 provider 随后照样签发令牌，实测禁用账号仍返回 200 |
| Refresh token 重放 | 判定泄漏，撤销该用户全部 token。实现会保存已成功消费令牌的 SHA-256 指纹与不可变用户 UUID（不落库原始令牌），同一令牌第二次提交时在用户行锁内级联删除该用户全部授权；撤销事务提交后再返回 `invalid_grant`，并发重放也不能绕过 |
...
| 签发令牌时用户已不存在 | 返回标准 OAuth2 错误体（`invalid_grant`），不再是非标准 500。触发条件是账号在 refresh token 有效期内被删除 |
| 邮箱达到最大长度（320）| 全链路贯通。**注意**：框架官方 schema 的 `oauth2_authorization.principal_name` 是 `varchar(200)`，照抄会让长邮箱账号「注册成功、登录成功、打开任何产品 500」且永久卡死。已由迁移放宽至 320，并有断言把「所有存邮箱的列宽」与 `EmailAddress.MAX_LENGTH` 绑在一起 |
| 已注册但未开放刷新授权的公有客户端来刷新 | 被拒绝，但最终错误码是 `invalid_grant` 而非自定义 provider 抛出的 `invalid_client`——`ProviderManager` 在自定义 provider 抛异常后仍会继续走框架的 provider，保留的是最后一个异常。属错误语义瑕疵，不影响拒绝结果 |
...
**删除语义**：同步只做增量 upsert。从配置中移除某个 client **不会**删除库中已注册的行及其回调白名单，下线产品需手动清理数据库（或待管理后台支持）。此语义与 `admin-emails` 的撤销语义一致。
