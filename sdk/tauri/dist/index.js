...
const AUTH_ERROR_CODES = new Set(TAURI_AUTH_ERROR_CODES);
...
export class TauriAuthError extends Error {
    code;
    constructor(code, message = AUTH_ERROR_MESSAGES[code], options) {
...
function normalizeAuthError(error, fallbackCode) {
    ...
    if (isRecord(error)
        ...
        return new TauriAuthError(error.code, error.message, { cause: error });
    }
...
export class TauriAuthClient {
    ...
    login(options = {}) {
        if (this.#logoutInFlight !== undefined) {
            return Promise.reject(new TauriAuthError('loginInProgress', '退出登录正在进行'));
        }
        if (this.#loginInFlight !== undefined) {
            if (this.#loginInFlight.prompt === options.prompt) {
                return this.#loginInFlight.operation;
            }
            return Promise.reject(new TauriAuthError('loginInProgress', '已有不同选项的登录流程正在进行'));
        }
    ...
    async getAccessToken() {
        ...
        const generation = this.#authGeneration;
        try {
            const accessToken = await this.#invoke(GET_ACCESS_TOKEN_COMMAND);
            this.#assertCurrentGeneration(generation);
            this.#setAuthenticated(true, generation);
            this.#assertCurrentGeneration(generation);
            return accessToken;
    ...
    #setAuthenticated(authenticated, generation) {
